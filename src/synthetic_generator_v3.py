...
#
# FITTING_ADD
#   → AR 피팅에 추가한 stronger positive
#
# FITTING_REMOVE
#   → 피팅을 시도한 뒤 현재 착용 상태에서 제거한 mild negative
#
# WISHLIST_ADD
#   → 강한 긍정
#
...
def generate_actions(
    ...
    fitting_readded = False
    if fitted:
        # Removal is only possible after ADD and becomes less likely as
        # affinity rises. A later ADD models trying the same item on again.
        ...
        if fitting_removed:
            fitting_readd_probability = sigmoid(
                logit(0.16)
                + 1.5 * (affinity - 0.5)
            )
            fitting_readded = (
                random.random()
                < fitting_readd_probability
            )
    ...
    if wishlist_added:
        ...
        if wishlist_removed:
            # A small re-add probability preserves realistic state toggles
            # without making repeated wishlist changes dominant.
            wishlist_readd_probability = sigmoid(
                logit(0.08)
                + 1.5 * (affinity - 0.5)
            )
            wishlist_readded = (
                random.random()
                < wishlist_readd_probability
            )
    ...
    if fitted:
        actions.append("FITTING_ADD")

    if fitting_removed:
        actions.append("FITTING_REMOVE")
